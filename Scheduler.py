...
tasks = []
with open("storage.csv", "r", newline="") as csvfile:
...
def print_to_do_list(to_do_list):
    for index, task in enumerate(to_do_list):
        if task["completed"] == "not completed":
            print(
                f"{index}. {task["Name"]} at {task["scheduled time"]} is {task["completed"]}"
            )
        else:
            print(
                f"{index}. {task["Name"]} at {task["scheduled time"]} was {task["completed"]} at {task["completed time"]}"
            )


while True:
    choice = input(
        "view (v), Add Tasks (a), Quit (q), completed (c), remove task (r) > "
    )
    if choice == "v":
